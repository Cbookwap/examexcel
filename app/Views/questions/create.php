...
<div class="card border-0 shadow-sm">
    ...
    <div class="card-body">
        <form method="POST" action="<?= base_url('questions/create') ?>" enctype="multipart/form-data">
            <?= csrf_field() ?>
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control <?= $validation->hasError('question_text') ? 'is-invalid' : '' ?>"
                          ...
                          onblur="validateQuestionText(this)"><?= old('question_text') ?></textarea>
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control <?= $validation->hasError('points') ? 'is-invalid' : '' ?>"
                               name="points" value="<?= old('points', 1) ?>" min="1" max="100" required>
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control" name="time_limit"
                               value="<?= old('time_limit') ?>" min="0" placeholder="Optional">
                    </div>
                </div>
            </div>
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control" name="hints" rows="2"
                          placeholder="Provide hints for students..."><?= old('hints') ?></textarea>
            </div>
...
function checkForDuplicates() {
    ...
    // Add term_id and exam_type_id for proper duplicate checking
    ...
    // Debug logging
    console.log('Duplicate check data (create.php):', {
    ...
    fetch('<?= base_url('questions/check-duplicate') ?>', {
        method: 'POST',
        body: formData,
        headers: {
    ...
    .then(data => {
        console.log('Duplicate check response (create.php):', data);
        ...
        if (data.is_duplicate) {
            ...
            alertDiv.className = 'alert alert-danger duplicate-alert mt-2';
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (form) {
        form.addEventListener('submit', function(e) {
            ...
            if (duplicateAlert) {
                e.preventDefault();
                alert('Cannot submit: A duplicate question exists. Please modify your question or check the existing question.');
                return false;
            }
        });
    }
