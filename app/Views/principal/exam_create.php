...
<style>
    ...
    .form-check-card .card:hover {
        border-color: var(--theme-color);
        box-shadow: 0 2px 8px rgba(var(--theme-color-rgb), 0.15);
    }
...
<div class="row">
    <div class="col-12">
        ...
        <form method="POST" action="<?= base_url('principal/exams/create') ?>" id="examForm" class="needs-validation" novalidate>
            <?= csrf_field() ?>
                ...
                <div class="section-content">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" id="description" name="description" rows="3"
                                  placeholder="Optional description for the exam"><?= old('description') ?></textarea>
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-check-card">
                                    <input type="radio" id="single_subject" name="exam_mode" value="single_subject" 
                                           <?= old('exam_mode', 'single_subject') == 'single_subject' ? 'checked' : '' ?>>
                                    <label for="single_subject">
                                        <div class="card">
                                            <div class="card-body text-center">
                                                <i class="material-symbols-rounded mb-2" style="font-size: 32px; color: var(--theme-color);">book</i>
                                                <h6 class="fw-semibold">Single Subject</h6>
                                                <p class="text-muted small mb-0">
                                                    Exam focused on one subject with questions from that subject only
                                                </p>
                                            </div>
                                        </div>
                                    </label>
                            ...
                            <div class="col-md-6">
                                <div class="form-check-card">
                                    <input type="radio" id="multi_subject" name="exam_mode" value="multi_subject"
                                           <?= old('exam_mode') == 'multi_subject' ? 'checked' : '' ?>>
                                    <label for="multi_subject">
                                        <div class="card">
                                            <div class="card-body text-center">
                                                <i class="material-symbols-rounded mb-2" style="font-size: 32px; color: var(--theme-color);">library_books</i>
                                                <h6 class="fw-semibold">Multi-Subject</h6>
                                                <p class="text-muted small mb-0">
                                                    Comprehensive exam covering multiple subjects with separate sections
                                                </p>
                                            </div>
                                        </div>
                                    </label>
                            ...
                            <select class="form-select" id="exam_type" name="exam_type" required>
                                ...
                                <?php if (!empty($examTypes)): ?>
                                    <?php foreach ($examTypes as $type): ?>
                                        <option value="<?= esc($type['code']) ?>" <?= old('exam_type') == $type['code'] ? 'selected' : '' ?>>
                                            <?= esc($type['name']) ?>
                                        </option>
                                    <?php endforeach; ?>
                                <?php else: ?>
                    ...
                    <div class="row">
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="number" class="form-control" id="total_marks" name="total_marks"
                                   value="<?= old('total_marks', '100') ?>" min="1" max="1000" required>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="number" class="form-control" id="negative_marks_per_question" name="negative_marks_per_question"
                                   value="<?= old('negative_marks_per_question', '0.25') ?>" step="0.25" min="0" max="10" disabled>
            ...
            <div class="d-flex justify-content-end gap-3 mb-4">
                <a href="<?= base_url('principal/exams') ?>" class="btn btn-outline-secondary">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function handleExamModeChange() {
        const singleSubject = document.getElementById('single_subject').checked;
